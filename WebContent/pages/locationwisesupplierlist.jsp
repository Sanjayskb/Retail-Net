...
$(document).ready(function(){
	...
	$('[id^=city]').click(function() {
		var id = $(this).attr('id');
		var countId = id.substr(4,id.length);
		var value = $('#city'+countId).val();
		console.log(value);
		
		var inputData = "city_id="+value;
		$('#filtercity').attr('action', '${pageContext.request.contextPath}/sector/filterbycity/'+value);
...
<!--                     <li class="filter-list"><input class="pixel-radio" type="radio" id="food" name="brand"><label for="food">Food<span> (3600)</span></label></li> -->
                  ...
                  <c:forEach var="city" items="${cities}">
                   <c:set var="count" value="${count + 1}" scope="page" />
                    <li class="filter-list"><input class="pixel-radio" type="radio" id="city${count}" name="cities" value="${city.cityId}"><label for="${city.cityName}">${city.cityName}<span>(3600)</span></label></li>
                    </c:forEach>
                  </ul>
                </form>
              </li>
...
<!--             </div> -->
          ...
          <section class="lattest-product-area pb-40 category-list">
            <c:forEach items="${cities }" var="city2">
            <h3 style="color:white;"><c:out value="${city2.cityName }"/></h3>
            <div class="row">
            <c:forEach var="firm" items="${firm}">
            <c:if test="${firm.cities.cityName == city2.cityName }">
<!--               <div class="sup1" > -->
<!--     			<div class="item-main" > -->
